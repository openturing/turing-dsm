	...
	public static void main(String[] args) 
	{
		...
		response.getFacetFields().forEach(facetFields -> {
			...
			facetFields.getValues().forEach(
					facetField -> System.out.println(facetField.getLabel() + facetField.getApiURL()  + "(" + facetField.getCount() + ")"));
		});
		System.out.println("");
		System.out.println("---");
...
//		});
		...
		/*		
		for (TurSNDocument document : turSNResults.getTurSNDocuments())
		{
			Map doc = document.getContent().getFields();
			for (Object keyObj : doc.keySet())
			{
				String key = (String) keyObj;
				//String value = (doc.get(key) != null) ? (String) doc.get(key) : "";
				System.out.println("<BR>" + key + " = " + doc.get(key));
